...
interface EditPurchaseRequestProps {
  onClose: () => void;
  onEditPurchaseRequest: (formData: NewBudgetRequest) => void;
  currentUser: string;
  purchaseRequest: any; // The existing purchase request data to edit
}
...
const EditPurchaseRequest: React.FC<EditPurchaseRequestProps> = ({
  ...
  // Initialize form data with existing purchase request data
  const [formData, setFormData] = useState({
    title: purchaseRequest?.itemName || '', // Map itemName to title
    description: purchaseRequest?.requestPurpose || '', // Map requestPurpose to description
    department: 'Operations', // Auto-filled
    requester_name: 'Admin User', // Auto-filled
    requester_position: 'Administrator', // Auto-filled
    request_date: new Date().toISOString().split('T')[0], // Keep current date
    budget_period: purchaseRequest?.requestType || 'normal', // Map requestType to budget_period
    total_amount: (purchaseRequest?.quantity || 1) * (purchaseRequest?.unitPrice || 0),
    start_date: new Date().toISOString().split('T')[0], // Auto-set to today
    end_date: '', // Will be set from existing data if available
    created_by: currentUser
  });

  // Initialize items with existing purchase request data
  const [items, setItems] = useState<Item[]>([
    {
      ...
      subtotal: (purchaseRequest?.quantity || 1) * (purchaseRequest?.unitPrice || 0),
      type: 'supply' as 'supply' | 'service'
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent, saveAsDraft: boolean = false) => {
    ...
    const action = saveAsDraft ? 'save changes as draft' : 'update purchase request';
    const result = await showConfirmation(
      `Are you sure you want to ${action}?`,
      `Confirm ${saveAsDraft ? 'Draft' : 'Update'}`
    );
    ...
    if (result.isConfirmed) {
      try {
        ...
        await onEditPurchaseRequest(payload);
        showSuccess(
          `Purchase request ${saveAsDraft ? 'saved as draft' : 'updated'} successfully`, 
          'Success'
        );
        onClose();
      } catch (error: unknown) {
        ...
        showError('Failed to update purchase request: ' + errorMessage, 'Error');
      }
    }
  };
